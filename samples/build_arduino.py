...
#
#    Codigo adaptado de Ben Sasson, disponivel em http://playground.arduino.cc/BuildArduino/Py, acessado em 20/05/2013. 
#
#    Necessario Phyton e Software Arduino. Testado com Phyton 3.3.2 64 bits e Arduino 1.0.5.
#
# 	 A compilacao e gravacao eh executada atraves de chamada de linha de comando, com suporte para Windows e Linux. Exemplo de chamada de linha de comando abaixo.
#    
# 	 Exemplo de linha de comando no Windows para executar a compilacao do codigo e grava-lo na placa:
# 		python build_arduino.py -u COM9 -Warduino-1.0.5/hardware/arduino/cores/arduino/ -iarduino-1.0.5/hardware/arduino/variants/standard/  --baud 115200 --avr-path arduino-1.0.5\hardware\tools\avr\bin --dude-conf arduino-1.0.5\hardware\tools\avr\etc\avrdude.conf
#
# 	 Exemplo de linha de comando no Linux para executar a compilacao do codigo e grava-lo na placa:
# 		python build_arduino.py -u /dev/ttyACM0 -W ../../arduino-1.0.5/hardware/arduino/cores/arduino/ -i ../../arduino-1.0.5/hardware/arduino/variants/standard/  --baud 115200
#
#
...
def compile_source(source, avr_path='', target_dir=None, arch=ARCH, clock=CPU_CLOCK, include_dirs=[], verbose=False, simulate=False, core_files=''):
    """
    ...
    env = dict(source=source, target=target, arch=arch, clock=clock, env_version=ENV_VERSION, compiler=compiler, avr_path=avr_path, core_files=core_files)
    ...
    if ext == '.ino':
    	cmdline = '%(avr_path)s%(compiler)s -c %(verbose)s -Wall -fno-exceptions -ffunction-sections -fdata-sections -funsigned-char -funsigned-bitfields -fpack-struct -fshort-enums -g -Os -w -ffunction-sections -fdata-sections -mmcu=%(arch)s -DF_CPU=%(clock)dL -DARDUINO=%(env_version)d -Iutil -Iutility %(include_dirs)s -x c++ -include %(core_files)sArduino.h %(source)s -o%(target)s' % env
    else:
    	cmdline = '%(avr_path)s%(compiler)s -c %(verbose)s -Wall -fno-exceptions -ffunction-sections -fdata-sections -funsigned-char -funsigned-bitfields -fpack-struct -fshort-enums -g -Os -w -ffunction-sections -fdata-sections -mmcu=%(arch)s -DF_CPU=%(clock)dL -DARDUINO=%(env_version)d -Iutil -Iutility %(include_dirs)s %(source)s -o%(target)s' % env
    _exec(cmdline, simulate=simulate)
    return target


def compile_directory(directory, target_dir=None, include_dirs=[], avr_path='', arch=ARCH, clock=CPU_CLOCK, verbose=False, simulate=False, core_files=''):
    """
    ...
    for fname in os.listdir(directory):
        if os.path.isfile(os.path.join(directory, fname)):
            obj_files.append(compile_source(os.path.join(directory, fname), include_dirs=include_dirs, avr_path=avr_path, target_dir=target_dir, arch=arch, clock=clock, verbose=verbose, simulate=simulate, core_files=core_files))
...
def main(argv):
    ...
    core_obj_files = compile_directory(core_files, build_directory, include_dirs=(options.libraries + options.include_dirs + [core_files]), avr_path=avr_path, arch=options.arch, clock=options.cpu_clock, verbose=options.verbose, simulate=options.simulate)
   
    
    # compile directories passed to program
    libraries_obj_files = []
    for library in options.libraries:
        libraries_obj_files.extend(compile_directory(library, build_directory, include_dirs=options.libraries + options.include_dirs + [core_files], avr_path=avr_path, arch=options.arch, clock=options.cpu_clock, verbose=options.verbose, simulate=options.simulate))

    # compile project
    project_obj_files = compile_directory(options.directory, build_directory, include_dirs=(options.include_dirs + options.libraries + [core_files]), avr_path=avr_path, arch=options.arch, clock=options.cpu_clock, verbose=options.verbose, simulate=options.simulate,core_files=options.wprogram_directory)
    ...
    project_obj = ''.join(project_obj_files)
    libraries_obj_files.append(project_obj)
    libraries_obj_files.extend(core_obj_files)
    link(link_output, libraries_obj_files, avr_path=avr_path, verbose=options.verbose, simulate=options.simulate)
